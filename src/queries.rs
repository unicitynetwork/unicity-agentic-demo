use crate::models::{Agent, CreateAgent, CreateMethod, Method, Port, Record};
use std::sync::Arc;
...
impl Queries {
    ...
    pub async fn create_method(&self, method: CreateMethod) -> Result<Record, anyhow::Error> {
        let created: Option<Record> = self.db.create("method").content(method).await?;
    ...
    pub async fn create_agent(&self, agent: CreateAgent) -> Result<Record, anyhow::Error> {
        let created: Option<Record> = self.db.create("agent").content(agent).await?;

        created.ok_or_else(|| anyhow::anyhow!("Failed to create agent"))
    }

    pub async fn relate_port(
        &self,
        from: RecordId,
        to: RecordId,
        port: Port,
    ...
    pub async fn get_transaction_history(
        ...
        let transactions: Vec<serde_json::Value> = self
            .db
            .query(&format!(
                "SELECT * FROM transaction ORDER BY created_at DESC LIMIT {}",
                limit
            ))
            .await?
            .take(0)?;
